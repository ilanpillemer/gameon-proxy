...
wget --user=admin --password=$ADMIN_PASSWORD --no-check-certificate https://$PROXY_DOCKER_HOST:8443/dockerneeds.tar -q
...
wget --user=admin --password=$ADMIN_PASSWORD --no-check-certificate https://$PROXY_DOCKER_HOST:8443/proxy.pem -O proxy.pem
